   ...
   if(!empty($data))
         {
            $user=$this->CI->session->userdata('admin_username');
            $pass=$this->CI->session->userdata('admin_view_password');
            $cek_login=$this->log_rekap($user,$pass);
            
            if ($cek_login==false) {
               echo "<script>alert('Anda telah keluar dari sistem'); window.location='".base_url()."peta/rekap_data';</script>";
            }
         } else {
               echo "<script>alert('Anda harus login terlebih dahulu'); window.location='".base_url()."peta/rekap_data';</script>";
         }
      }   
   }